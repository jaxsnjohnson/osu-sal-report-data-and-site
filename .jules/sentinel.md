...
## 2024-05-27 - DOM XSS via unescaped string injection in innerHTML mapping
**Vulnerability:** In `js/app.js` (`renderSuggestedSearches`), an array of strings was being directly mapped and injected via string interpolation into `.innerHTML` as button text and inside an `onclick` inline event handler, without any escaping (e.g., `<button onclick="applySearch(${term})">\${term}</button>`).
**Learning:** Even if an array is currently hardcoded with safe values, string interpolations directly inside `innerHTML` assignments represent an inherent defense-in-depth failure. If the array ever became dynamic (like using previously searched terms), it would immediately introduce a critical XSS vector because the `term` string was not HTML-escaped.
**Prevention:** Always explicitly HTML-escape interpolated string variables used in `.innerHTML` array map calls, wrapping values for HTML attributes with `escapeHtmlAttr(escapeForSingleQuote(...))` and text content with `escapeHtml(...)`.
