...
## 2024-05-24 - Missing HTML attribute escaping in dynamically generated tooltips
**Vulnerability:** Dynamically generated strings for attributes like `data-tooltip` on elements such as `exclusion-warning` and `latest-salary` were injected into HTML without using `escapeHtmlAttr()`. While `exclusionTooltip` was hardcoded, `totalPayTooltip` depended on dynamically generated `person._payMissing`.
**Learning:** Even hardcoded strings and localized dynamic strings interpolated into HTML attributes should be wrapped in attribute escaping functions. This ensures future modifications (like adding user variables to these strings) do not inadvertently create XSS vectors.
**Prevention:** Consistently wrap all string variables injected into HTML attributes with an attribute-specific HTML escaper (like `escapeHtmlAttr()`) to establish a strong defense in depth posture.
